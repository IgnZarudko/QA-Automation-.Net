...
namespace ConsoleApp1.Tests
{
    ...
    public class FileTests
    {
        ...
        public void Constructor_NameIsCorrect(string name, string content)
        {
            File file = new File(name, content);
            Assert.AreEqual(name, FileInfoFinder.GetFileName(file));
        }

        [TestCase("filename.extension", "some strange content, does not matter.")]
        [TestCase(".gitignore", "*/idea")]
        [TestCase("empty-content-file.txt", "")]
        public void Constructor_ContentIsCorrect(string name, string content)
        {
            File file = new File(name, content);
            Assert.AreEqual(content, FileInfoFinder.GetFileContent(file));
        }
        ...
        public void Constructor_SizeIsCorrect(string name, string content, double expectedSize)
        {
            File file = new File(name, content);
            Assert.AreEqual(expectedSize, FileInfoFinder.GetFileSize(file));
        }
        ...
        public void Constructor_ExtensionIsCorrect(string name, string content, string expectedExtension)
        {
            File file = new File(name, content);
            Assert.AreEqual(expectedExtension, FileInfoFinder.GetFileExtension(file));
        }
    }
}